...
use bevy_hecs::{
    ArchetypeComponent, ComponentId, TypeAccess,
    World,
};
use std::borrow::Cow;
...
//             query.get_world_archetype_access(world, Some(component_access));

//             // Make sure the query doesn't collide with any existing queries
//             if let Some(archetype_component) =
//                 component_access.get_conflict(system_archetype_component_access)
//             {
